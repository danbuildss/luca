...
export async function formatQualityReport(userId: string): Promise<string> {
  ...
  lines.push('*Classification quality, last 7 days*', '');
  ...
  if (snapshot.high_confidence_errors > 0) {
    lines.push(`High-confidence errors: ${snapshot.high_confidence_errors} (labels trusted but wrong)`);
  }
  lines.push('');
  ...
  if (riskyLabels.length > 0) {
    ...
    for (const l of riskyLabels.slice(0, 5)) {
      const star = l.precision_proxy < 0.9 ? 'High' : 'Watch';
      lines.push(
  ...
  if (badMethods.length > 0) {
    ...
    for (const m of badMethods) {
      const icon = m.error_rate > 0.1 ? 'High' : m.error_rate > 0.05 ? 'Watch' : 'OK';
  ...
  if (weeklyTrend.length >= 2) {
    ...
    for (const w of recent) {
      ...
      const corrIcon = w.correction_rate > 0.1 ? 'High' : w.correction_rate > 0.05 ? 'Watch' : 'OK';
      lines.push(
  ...
  if (goldSummary.length > 0) {
    ...
    const passIcon = overallPassRate >= 0.95 ? 'OK' : overallPassRate >= 0.85 ? 'Watch' : 'High';
    ...
    if (failing.length > 0) {
      for (const r of failing.slice(0, 5)) {
        lines.push(`  ${r.pass_rate < 0.8 ? 'High ' : 'Watch'} ${r.correct_label.padEnd(18)} ${pct(r.pass_rate)}  (${r.correct}/${r.total})`);
      }
    }
    lines.push('');
  }
  ...
  if (deterministicErrors) {
    actions.push(`Audit deterministic rules: a ${pct(deterministicErrors.error_rate)} error rate is too high for rule-based logic`);
  }
