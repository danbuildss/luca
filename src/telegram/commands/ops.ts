...
async function handleOpsOverview(ctx: Context): Promise<void> {
  ...
  const lines = [
    ...
    ``,
    `*Users*`,
    `  Total: ${ov.users.total}  |  Activated: ${ov.users.activated}  |  Active 24h: ${ov.users.active_24h}  |  7d: ${ov.users.active_7d}`,
    ``,
    `*Invites*`,
    `  Invited: ${ov.invites.invited}  |  Pending: ${ov.invites.pending}  |  Joined: ${ov.invites.joined}  |  Activated: ${ov.invites.activated}  |  Revoked: ${ov.invites.revoked}`,
    ``,
    `*Wallets*`,
    `  Monitored: ${ov.wallets.monitored}  |  OK: ${ov.wallets.ok}  |  Stale: ${ov.wallets.stale}  |  Error: ${ov.wallets.error}  |  Inactive: ${ov.wallets.inactive}`,
    ``,
    `*Worker*`,
